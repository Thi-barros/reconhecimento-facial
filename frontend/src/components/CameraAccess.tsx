...
const CameraAccess: React.FC<CameraAccessProps> = ({ onAccessResult }) => {
  ...
  const isLocked = lockRemaining !== null; // 🔒 estado derivado
  ...
  const checkAccess = useCallback(async () => {
    ...
    try {
      ...
      // 🔒 Tratamento de bloqueio: mostrar só o alerta com contador
      if (result.locked) {
        setLastResult(null); // não mostrar card de resultado durante bloqueio
        const initial = result.lock_remaining_seconds ?? 60;
        setLockRemaining(initial);

        const id = window.setInterval(() => {
          setLockRemaining((prev) => {
            if (prev === null || prev <= 1) {
              window.clearInterval(id);
              return null; // desbloqueia visualmente
            }
            return prev - 1;
          });
        }, 1000);
    ...
    <Card sx={{ maxWidth: 500, margin: 'auto', mt: 4 }}>
      <CardContent>
        ...
        <Box sx={{ display: 'flex', gap: 2, justifyContent: 'center', mb: 2 }}>
          {/* Esconde o botão "Ativar Câmera" durante bloqueio para não confundir */}
          {!isCameraActive ? (
            !isLocked && (
              <Button
                variant="contained"
            ...
            <Button
              ...
              onClick={stopCamera}
              size="large"
              disabled={isLocked} // opcional: desabilitar "Parar" durante bloqueio
            >
              Parar Câmera
            </Button>
          )}
          ...
          <Button
            ...
            color="primary"
            onClick={checkAccess}
            disabled={isProcessing || !isCameraActive || isLocked}
            size="large"
          >
            {isProcessing ? (
              <>
            ...
            ) : isLocked ? `BLOQUEADO (${lockRemaining ?? 0}s)` : 'Verificar Acesso'}
          </Button>
        </Box>

        {/* Aviso ÚNICO de bloqueio (com contador) */}
        {isLocked && (
          <Alert severity="warning" sx={{ mt: 2 }}>
            Muitas tentativas inválidas. A verificação está bloqueada por segurança. Aguarde {lockRemaining ?? 0}s.
          </Alert>
        )}

        {/* Resultado NORMAL — só quando NÃO estiver bloqueado */}
        {!isLocked && lastResult && (
        ...
        {/* Erros reais (rede/back) — não mostrar durante bloqueio */}
        {!isLocked && error && (
